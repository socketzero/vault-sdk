...
const CONNECTION_A = "018f2a3b-0000-7000-8000-000000000001";
const CONNECTION_B = "018f2a3b-0000-7000-8000-000000000002";
...
describe("associated data", () => {
  ...
  it("binds a field as AAD(connection_uuid, field_name)", () => {
    const uuid = parseUuid(CONNECTION_A);
    const aad = fieldAssociatedData(uuid, "password");
    // The connection component is the raw sixteen bytes, so its length prefix
    // is always 16 — a rendered id never reaches the cipher.
    expect(Array.from(aad.subarray(0, 4))).toEqual([0, 0, 0, 16]);
    expect(Array.from(aad.subarray(4, 20))).toEqual(Array.from(uuid));
  });

  it("is one length prefix plus one body per component", () => {
    const aad = fieldAssociatedData(parseUuid(CONNECTION_A), "password");
    expect(aad).toHaveLength(2 * AAD_LENGTH_PREFIX_BYTES + 16 + "password".length);
  });

  it("distinguishes two field names under one connection", () => {
    expect(fieldAssociatedData(parseUuid(CONNECTION_A), "password")).not.toEqual(
      fieldAssociatedData(parseUuid(CONNECTION_A), "api_key"),
    );
  });

  it("distinguishes two connections under one field name", () => {
    expect(fieldAssociatedData(parseUuid(CONNECTION_A), "password")).not.toEqual(
      fieldAssociatedData(parseUuid(CONNECTION_B), "password"),
    );
  });

  it("refuses a connection component that is not sixteen bytes", () => {
    // A display string cannot be passed by type, but a wrong-sized byte array
    // still can, and it would silently bind a different identity.
    expect(() => fieldAssociatedData(new Uint8Array(15), "password")).toThrow(VaultError);
    expect(() => fieldAssociatedData(new Uint8Array(17), "password")).toThrow(/16 raw UUID bytes/);
  });
  ...
  // The string-component semantics live on the bucket AAD now: it is the one
  // that still binds two rendered identifiers.
  it("prefixes the UTF-8 byte length, not the code-unit count", () => {
    // "é" is one character and two UTF-8 bytes; "ß" likewise.
    expect(bucketAssociatedData("é", "ß")).toEqual(expectedAad("é", "ß"));
    expect(Array.from(bucketAssociatedData("é", "ß"))).toEqual([
      0, 0, 0, 2, 0xc3, 0xa9, 0, 0, 0, 2, 0xc3, 0x9f,
    ]);
  });

  it("accepts an empty component and still prefixes it", () => {
    expect(bucketAssociatedData("", "")).toEqual(Uint8Array.from([0, 0, 0, 0, 0, 0, 0, 0]));
  });

  it("does not canonicalise, trim or lower-case a component", () => {
    expect(bucketAssociatedData("Grp", " 0a1b ")).toEqual(expectedAad("Grp", " 0a1b "));
    expect(bucketAssociatedData("Grp", "0a1b")).not.toEqual(bucketAssociatedData("grp", "0a1b"));
  });

  it("separates the shift that plain concatenation collided", () => {
    // The whole reason the prefix exists: `("grp","10a1b")` and `("grp1","0a1b")`
    // concatenate to the same bytes.
    expect(utf8.encode("grp10a1b")).toEqual(utf8.encode("grp10a1b"));
...
describe("seal / open round trip", () => {
  ...
  it("accepts the public half explicitly, skipping the base-point derivation", async () => {
    const group = await generateX25519KeyPair();
    const aad = fieldAssociatedData(parseUuid(CONNECTION_A), "token");
...
describe("open rejects everything that is not the exact original", () => {
  it("fails on a different AAD field name", async () => {
    ...
    const envelope = await seal(
      ...
      group.publicKey,
      fieldAssociatedData(parseUuid(CONNECTION_A), "note"),
    );

    await expect(
      open(envelope, group.privateKey, fieldAssociatedData(parseUuid(CONNECTION_A), "api_key")),
  ...
  it("fails when the envelope is moved to another connection", async () => {
    ...
    const envelope = await seal(
      utf8.encode("secret"),
      group.publicKey,
      fieldAssociatedData(parseUuid(CONNECTION_A), "password"),
    );

    await expect(
      open(envelope, group.privateKey, fieldAssociatedData(parseUuid(CONNECTION_B), "password")),
    ).rejects.toBeInstanceOf(VaultDecryptionError);
  });
...
describe("open contains the whole derive path in the failure taxonomy", () => {
  ...
  it.each(LOW_ORDER_POINTS.map((point, i) => [i, point] as const))(
    ...
    async (_i, point) => {
      const pair = await generateX25519KeyPair();
      const aad = fieldAssociatedData(parseUuid(CONNECTION_A), "field");
